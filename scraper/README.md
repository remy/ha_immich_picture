...
2. (Optional) Adjust the `scripts_dir` option if you prefer saving uploads somewhere other than the default `/config/scripts` (for example `/share/scraper`).
3. (Optional) Add `env_vars` key/value pairs (API keys, credentials, etc.). They are exported to the Node process as environment variables on startup. A sample `EXAMPLE_API_KEY` entry is provided—replace it or add more secrets as needed.
3. Start the add-on and open it via the Ingress **OPEN WEB UI** button or by browsing to `http://<home-assistant-host>:<mapped-port>/`.
